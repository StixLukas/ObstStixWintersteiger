...
void readBME280(bme280_t *bme) {
  ...
  bme->temperature = compensateTemperature(t_fine); // C
  bme->pressure = compensatePressure(raw.pressure, &cal, t_fine) / 100; // hPa
  bme->humidity = compensateHumidity(raw.humidity, &cal, t_fine);       // %
  bme->altitude = getAltitude(p);                         // meters
  }
}
